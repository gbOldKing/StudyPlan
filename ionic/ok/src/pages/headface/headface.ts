...
export class HeadfacePage extends BaseUI {
  ...
  takePicture(sourceType) {
    ...
    this.camera.getPicture(options).then((imagePath) => {
      ...
      super.showTast(this.toastCtrl, "选择图片出现错误，请在 App 中操作或检查相关权限。");
    });
  }
  ...
  copyFileToLocalDir(namePath, currentName, newFileName) {
    this.file.copyFile(namePath, currentName, cordova.file.dataDirectory, newFileName).then(success => {
      ...
      super.showTast(this.toastCtrl, "存储图片到本地图库出现错误。");
    });
  }
  ...
  public pathForImage(img) {
    if (img === null) {
      ...
      //normalizeUrl fix ios11 camera no working;
  ...
  uploadImage() {
    ...
    fileTransfer.upload(targetPath, url, options).then(data => {
      loading.dismiss();
      super.showTast(this.toastCtrl, "图片上传成功。");
      //在用户看清弹窗提示后进行页面的关闭
      setTimeout(() => {
      ...
      loading.dismiss();
      super.showTast(this.toastCtrl, "图片上传发生错误，请重试。");
    });
  }
}
